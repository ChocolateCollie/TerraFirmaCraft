    ...
    public static void registerColorHandlerBlocks(ColorHandlerEvent.Block event)
    {
        ...
        IBlockColor grassColor = (state, worldIn, pos, tintIndex) -> {
            if (worldIn != null && pos != null)
            {
                ...
                try
                {
                    Field worldField = worldIn.getClass().getDeclaredField("world");
                    worldField.setAccessible(true);
                    world = (World) worldField.get(worldIn);
                }
                catch (Exception e)
                {
                    TerraFirmaCraft.getLog().info("PROBLEMS!!!", e);
                }
                if (world != null)
                {
                    ChunkDataTFC data = ChunkDataTFC.get(world, pos);
                    double temp = MathHelper.clamp((data.getAverageTemp() + 15) / 50, 0, 1);
                    double rain = MathHelper.clamp(data.getRainfall() / 500, 0, 1);
        ...
        IBlockColor foliageColor = (state, worldIn, pos, tintIndex) -> {
            if (worldIn instanceof ChunkCache && pos != null)
            {
                World world = null;
                try
                {
